...
def show_pred_and_img(data, label, prediction, iters=5, rows=3, cols=4, write_rate=None):
    plt.figure()
    for _ in range(iters):
        for i in range(rows * cols):
            ...
            if write_rate is not None:
                print(f"{label[n]}: {[round(a, 4) for a in write_rate[n]]}")
        plt.show()


class CnnCifar10:
    def __init__(self, epochs=100, learning_rate=0.001, batch_size=100, num_outputs=10):
        self.epochs = epochs
        self.learning_rate = learning_rate
        self.num_outputs = num_outputs  # 10 classes
        ...
        self.path = './models/cifar/model_cif10_v2(dropout).ckpt'
    ...
    def init_weights(self):
        ...
        # Convert layer 1_2
        self.W1_2 = tf.Variable(tf.truncated_normal([4, 4, 64, 64], dtype=tf.float32, stddev=1e-1), name='weights1_2')
        self.b1_2 = tf.Variable(tf.constant(0.0, shape=[64], dtype=tf.float32), trainable=True, name='biases1_2')
        # # Convert layer 1_3
        # self.W1_3 = tf.Variable(tf.truncated_normal([3, 3, 64, 64], dtype=tf.float32, stddev=0.1), name='weights1_3')
        # self.b1_3 = tf.Variable(tf.constant(0.0, shape=[64], dtype=tf.float32), trainable=True, name='biases1_3')
        ...
        # # Convert layer 2_3
        # self.W2_3 = tf.Variable(tf.truncated_normal([3, 3, 128, 192], dtype=tf.float32, stddev=0.1), name='weights23')
        # self.b2_3 = tf.Variable(tf.constant(0.0, shape=[192], dtype=tf.float32), trainable=True, name='biases2_3')
        # # Convert layer 3_1
        # self.W3_1 = tf.Variable(tf.truncated_normal([2, 2, 128, 256], dtype=tf.float32, stddev=0.1), name='weights31')
        # self.b3_1 = tf.Variable(tf.constant(0.0, shape=[256], dtype=tf.float32), trainable=True, name='biases3_1')

        # Hidden layer 1 (512)
        self.hW1 = tf.Variable(tf.truncated_normal([8*8*128, 512], dtype=tf.float32, stddev=1e-1), name='hid_weights1')
        self.hb1 = tf.Variable(tf.constant(0.0, shape=[512], dtype=tf.float32), trainable=True, name='hid_biases1')
        # Hidden layer 2  (128)
        self.hW2 = tf.Variable(tf.truncated_normal([512, 128], dtype=tf.float32, stddev=1e-1), name='hid_weights2')
        self.hb2 = tf.Variable(tf.constant(0.0, shape=[128], dtype=tf.float32), trainable=True, name='hid_biases2')
        # Output layer
        self.W_out = tf.Variable(tf.truncated_normal([128, self.num_outputs], dtype=tf.float32, stddev=1e-1),
                                 name='weights_out')
    ...
    def model_op(self):
        ...
        # conv_1_3 = self.conv_layer(conv_1_2, self.W1_3, self.b1_3)  # Convert layer 1_3
        ...
        # conv_2_3 = self.conv_layer(conv_2_2, self.W2_3, self.b2_3)  # Convert layer 2_23  + 3s
        max_pool_2 = self.maxpool_layer(conv_2_2, k=2)  # Pooling 2  16x16 -> 8x8
        # conv_3_1 = self.conv_layer(max_pool_2, self.W3_1, self.b3_1)  # Convert layer 3_1
        # max_pool_3 = self.maxpool_layer(conv_3_1, k=2)  # Pooling 3  8x8 -> 4x4
    ...
    def test(self, data, labels, sess):
        ...
        prediction, rating, accuracy, loss = np.zeros(len(labels)), np.zeros((len(labels), self.num_outputs)), [], 0
        for i in range(0, len(data), self.batch_size):
            batch_data = data[i:i + self.batch_size]
            batch_labels = one_hot_labels[i:i + self.batch_size]
            pred, acc, lo, rate = sess.run([tf.cast(tf.argmax(self.model, axis=1), tf.int8), self.accuracy,
                                            self.cost, self.prob],
            ...
            rating[i:i + self.batch_size, :] = rate
        return prediction, rating, sum(accuracy) / len(accuracy), loss
            ...
            for epoch in range(1, self.epochs+1):
                ...
                for i in range(0, len(data), self.batch_size):
                    ...
                    progress_bar(i, len(data), prefix=f"EPOCH-{epoch}:\t",
                                 suffix=f"[{i}-{len(data)}]\tAccuracy: {accuracy_val * 100 :.2f}%  Loss: {loss:.2f}")

                    if i + self.batch_size == len(data):  # end for
                        progress_bar(1, 1, prefix=f"EPOCH-{epoch}:\t",
                                     suffix=f"[{len(data)}-{len(data)}]\tTime: {time() - s :.2f} sec")
                        _, _, accuracy_test, loss_test = self.test(test_data, test_labels, sess)
                        print(f"Accuracy: {accuracy_total / iters * 100 :.2f}%   Loss: {loss_total / 5:.3f}   "
                              f"Accuracy_test: {accuracy_test * 100 :.2f}%   Loss_test: {loss_test:.3f}")
...
if __name__ == "__main__":
    names, imgs_train, labels_train, imgs_test, labels_test = cifar_tools.read_data("../datasets/cifar/cifar-10-python")
    ...
    with tf.Session() as session:
        cnn.load(session)
        train_predict, train_rating_, train_accuracy_, _ = cnn.test(imgs_train, labels_train, session)
        predict, rating_, accuracy_, _ = cnn.test(imgs_test, labels_test, session)

    print(f"Accuracy train: {train_accuracy_ * 100 :.2f}%\nAccuracy: {accuracy_ * 100 :.2f}%")
    show_pred_and_img(imgs_test, labels_test, predict)
    show_pred_and_img(imgs_test, labels_test, predict, iters=15, rows=1, cols=1, write_rate=rating_)
