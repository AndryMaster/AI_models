...
class vgg16:
    def __init__(self, imgs, sess=None, weights=None):
        ...
        # self.my_writer = tf.summary.FileWriter('vgg_logs', sess.graph)
...
if __name__ == "__main__":
    ...
    img_laska = Image.open('laska.png')
    img_laska = np.array(Image.Image.resize(img_laska, (224, 224)))[:, :, :3]  # 3 channel ARGB -> RGB
    plt.imshow(img_laska)
    plt.title('Input 224x244 image')
    plt.show()

    print('Loading model...')
    vgg = vgg16(imgs, sess, 'vgg16_weights.npz')
    print('Done loading!')

    my_summaries = tf.summary.merge_all()
    my_writer = tf.summary.FileWriter('vgg_logs', sess.graph)

    prob, fc2_val, my_summaries_protobuf = sess.run([vgg.probs, vgg.fc2, my_summaries], feed_dict={vgg.imgs: [img_laska]})
    prob = prob[0]
    ...
    plt.title(f'{num_dimensions}-dimensional representation of image')
    plt.show()

    print('Top 5 predictions of VGG-16 model:')
    predicts = (np.argsort(prob)[::-1])[0:5]
    for idx, p in enumerate(predicts):
        print(f'{idx + 1}. {class_names[p]} ({prob[p]})')
    sess.close()

    # Top 5 predictions of VGG-16 model:
    # 1. weasel (0.7001557946205139) == laska
    # 2. polecat, fitch, foulmart, foumart, Mustela putorius (0.1703096181154251)
    # 3. mink (0.1225309744477272)
    # 4. black-footed ferret, ferret, Mustela nigripes (0.0067447274923324585)
    # 5. otter (0.00012196582974866033)
